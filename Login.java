public class Login {
    ...
    public boolean checkPasswordComplexity(String password){
        String captial = ".*[A-Z].*";//Check capital letters 
        String small = ".*[a-z].*";//Check small letters
        String special = ".*[!@#$%^&*(),.?\":{}|<>].*";//Check special charcters 
        String digit = ".*\\d.*";//Check digit characters 
        if(password.length() >= 8 && password.matches(captial) && password.matches(small) && password.matches(digit) && password.matches(special)){
            return true;
        }
        else{
            return false;
